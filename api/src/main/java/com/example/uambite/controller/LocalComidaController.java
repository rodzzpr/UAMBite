...
public class LocalComidaController {
    ...
    public ResponseEntity<Page<LocalComidaResponse>> getAll(Pageable pageable) {
    ...
    @PreAuthorize("hasRole('ADMIN') or @ownershipService.canEditLocal(#id, principal.id)")
    @Operation(summary = "Subir o reemplazar la imagen del local. ADMIN o dueño del local.")
    public ResponseEntity<ImagenResponse> uploadImagen(@PathVariable UUID id,
                                                       @RequestParam("file") MultipartFile file) {
        service.setImagen(id, file);
        return ResponseEntity.ok(ImagenResponse.of("/localcomida/" + id + "/imagen"));
    }

    @GetMapping("/{id}/imagen")
    @Operation(summary = "Obtener la imagen del local (público)")
    ...
    @PreAuthorize("hasRole('ADMIN') or @ownershipService.canEditLocal(#id, principal.id)")
    @Operation(summary = "Eliminar la imagen del local. ADMIN o dueño del local.")
